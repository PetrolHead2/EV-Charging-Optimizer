...
14a. **`initial:` on YAML input helpers resets on every HA restart** — any `input_boolean`, `input_number`, etc. with `initial:` in YAML is forced back to that value on every restart or platform reload, overriding user changes. All user-preference entities (`ev_tariff_guard_enabled`, `ev_auto_deadline`, `ev_max_hourly_kwh`, `ev_max_tariff_power_kw`, `ev_max_price_sek`) have had `initial:` removed. Transient-state flags (`ev_consumption_guard_active`, `ev_preclimate_charging`) keep `initial: false` because pyscript re-syncs them within minutes of startup.
...
45. **`input_select` vs `select` domain in pyscript** — `input_select.ev_charging_mode` is an `input_select` entity. The pyscript service call MUST use `input_select.select_option(entity_id=..., option=...)`. Calling `select.select_option(...)` targets the wrong domain and fails silently (no error logged, entity unchanged). Confirmed: HA REST API also uses `input_select/select_option` for these entities.
46. **`@state_trigger` does not fire retroactively** — if pyscript restarts while the watched entity is already in the target state, the trigger will not fire until the next state change. For `on_preclimate_changed`: a `preclimate_poll()` function runs every 2 minutes and catches the gap by checking flag vs sensor state and calling `on_preclimate_changed()` directly (same-file call, synchronous). Catch-up condition: `preclimate=on AND flag=off` → missed activation; `preclimate=off AND flag=on` → missed deactivation.
