...
4. **ev_schedule state is the JSON string**: `state.get("sensor.ev_schedule")` returns the JSON directly. In the control loop, parse it with `json.loads(sched_state)`. Do not try to read it from attributes. The state contains only `{"start", "end"}` per window (to stay within HA's 255-char state limit); full window data (price, slots, kwh, cost) is in `attributes.schedule` for Lovelace display. The `cost` field is per-window accurate cost (sum of `price × effective_power_kw × 0.25` per slot) — not flat `price × kwh`.
