...
16. **Two deadline entities: human input vs pyscript computed**: `input_datetime.ev_deadline` is the human-only manual override — pyscript never writes it. `input_datetime.ev_computed_deadline` is written only by `auto_set_deadline()` and never shown as editable in the UI. `get_effective_deadline()` picks the manual deadline if it is > 5 minutes in the future; otherwise falls back to the computed deadline; otherwise opportunistic mode. To set a manual one-off deadline: write `ev_deadline` to the desired time. To clear it and return to the weekly schedule: set `ev_deadline` to a past date (e.g. 2000-01-01) or the 2099 sentinel. The optimizer will then use `ev_computed_deadline` from the weekly schedule automatically.
