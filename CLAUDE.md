...
12. **Consumption guard projection**: projects kWh for the remainder of the current hour as `accumulated_kwh + (house_kw + ev_kw) × (remaining_minutes / 60)`. Both Tibber sensors (`accumulated_consumption_current_hour` and `average_power`) must be available — if either is `unavailable`/`unknown` the guard is skipped (fail-open). The `average_power` sensor reflects whole-house draw; since the EV is OFF at evaluation time, adding `ev_charging_power_kw` gives the correct projected total. Guard only runs when `desired == True` (i.e. the schedule says charge) — it is never applied to stops.
