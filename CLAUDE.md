...
The **control layer** (`ev_control_loop.py`) runs every 5 minutes, on pyscript startup, and immediately whenever `sensor.laddbox_charger_mode` transitions to `"charging"`. Additionally, `on_input_changed()` in `ev_optimizer.py` fires `ev_control_loop()` directly ~3 seconds after any input change (deadline, target, mode, schedule) — new windows or deadline pressure take effect within seconds rather than waiting for the next 5-minute tick. The reactive charge-start trigger (`ev_control_on_charge_start`) closes the gap between car connection (when Zaptec may auto-start) and the next scheduled tick, stopping unauthorised charging within seconds. All three entry points share the same decision tree: manual override modes (Charge now / Stop) take absolute priority; then it reads deadline pressure (`binary_sensor.ev_deadline_pressure`); the consumption guard always runs next (even during deadline pressure); step 5 handles the no-schedule case; step 6 forces ON immediately for deadline pressure **without hysteresis** — it is a hard override that must start charging regardless of how recently the last state change occurred; steps 7/8 follow the schedule with a 15-minute hysteresis guard. The consumption guard blocks charging if the projected hourly kWh would exceed `input_number.ev_max_hourly_kwh`. The guard only applies during Ellevio tariff hours (06:00–22:00 local time) and can be disabled via `input_boolean.ev_tariff_guard_enabled`. All state changes write a human-readable reason to `input_text.ev_decision_reason`.
...
42. **`schedule_watchdog()` skips recompute when deadline is imminent or passed (2026-04-28)**: The watchdog fires every 15 minutes and recomputes when the schedule is empty but a deadline exists. Before this fix it would fire repeatedly in the 30 minutes before departure (no eligible slots remain → recompute finds nothing → schedule stays empty → fires again). Fixed: skip recompute silently when `deadline_ts - now_ts < 1800` (< 30 min) or when the deadline has already passed. Both cases produce the debug log `"deadline imminent in N min / already passed, empty schedule expected — skipping"` at DEBUG level (not WARNING). Only fire the WARNING recompute when schedule is empty AND deadline is > 30 min away.

44. **Deadline pressure bypasses hysteresis at step 6 (2026-04-29)**: Step 6 of the priority chain forces charging ON when `binary_sensor.ev_deadline_pressure` is true and the consumption guard has cleared. Previously `check_hysteresis(True)` was called here, which could hold charging OFF for up to 15 minutes even during an emergency deadline. Removed: step 6 now calls `set_charger(True, ...)` directly. Hysteresis still applies at steps 7/8 (normal schedule following). **Do not add `check_hysteresis()` back to the deadline-pressure path** — it defeats the purpose of having an emergency override.

45. **`on_input_changed()` fires `ev_control_loop()` immediately after recompute (2026-04-29)**: After `ev_optimizer_recompute()` + 1 s settle time, `on_input_changed()` calls `ev_control_loop()` directly. This means any input change (deadline, target, mode) triggers a charging decision within ~3 s rather than waiting up to 5 min for the next tick. Pyscript shares a global function namespace across files, so `ev_control_loop` (defined in `ev_control_loop.py`) is callable from `ev_optimizer.py`. The 1 s sleep allows `sensor.ev_schedule` state to settle after the recompute write.
